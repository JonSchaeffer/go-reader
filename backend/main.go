...
func main() {
	...
	http.HandleFunc("/api/categories-with-feeds", corsMiddleware(routeCategoriesWithFeeds)) // Categories with feeds
...
func routeHighlightsAll(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		rss.GetAllHighlights(w, r)
	default:
		http.Error(w, "Method is not allowed or supported", http.StatusMethodNotAllowed)
	}
}
...
func routeCategoriesWithFeeds(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
